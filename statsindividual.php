...
class StatsIndividual extends Module
{
	private $html = '';
	private $t1 = 0;
	private $t2 = 0;
	...
	{
		$this->name = 'statsindividual';
		$this->tab = 'analytics_stats';
		$this->version = '1.0';
		$this->author = 'zimmer-media-office';
		$this->need_instance = 0;

		parent::__construct();

		$this->displayName = $this->l('Stats Individual');
		$this->description = $this->l('This is a individual module for the Stats.');
	...
	public function hookAdminStatsModules()
	{
		...
		SELECT
			...
			SUM(o.total_products_wt / o.conversion_rate) as totalGoods,
			SUM(o.total_shipping_tax_incl / o.conversion_rate) as totalShipping,
			SUM(o.total_wrapping_tax_incl / o.conversion_rate) as totalWrapping,
			SUM(o.total_discounts_tax_incl / o.conversion_rate) as totalDiscount,
		...
		$this->html .= '<div>
			...
			<div class="alert alert-info">'.$this->l('The listed amounts is include tax.').'</div>
			...
			<table class="table">
				<thead>
					<tr>
						...
						<th class="text-center"><span class="title_box active">'.$this->l('Goods').'</span></th>
						<th class="text-center"><span class="title_box active">'.$this->l('Shipping').'</span></th>
						<th class="text-center"><span class="title_box active">'.$this->l('Wrapping').'</span></th>
						<th class="text-center"><span class="title_box active">'.$this->l('Discounts').'</span></th>
		...
		foreach ($data_table as $row)
		{
			...
			<tr>
				...
				<td class="text-right">'.Tools::displayPrice($row['totalGoods'], $currency).'</td>
				<td class="text-right">'.Tools::displayPrice($row['totalShipping'], $currency).'</td>
				<td class="text-right">'.Tools::displayPrice($row['totalWrapping'], $currency).'</td>
				<td class="text-right">'.Tools::displayPrice($row['totalDiscount'], $currency).'</td>
			...
			$this->t5 += $row['totalGoods'];
			$this->t6 += $row['totalShipping'];
			$this->t7 += $row['totalWrapping'];
			$this->t8 += $row['totalDiscount'];
			$this->t9 += $row['totalSales'];
		}
		...
		$this->html .= '
				...
				<tr>
					...
					<td class="text-right">'.Tools::displayPrice($this->t5, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t6, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t7, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t8, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t9, $currency).'</td>
				</tr>
				<tr>
					...
					<td class="text-right">'.Tools::displayPrice($this->t5 / $interval_avg, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t6 / $interval_avg, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t7 / $interval_avg, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t8 / $interval_avg, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t9 / $interval_avg, $currency).'</td>
				</tr>
				<tr>
					...
					<td class="text-right">'.Tools::displayPrice($this->t5 * $prop30, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t6 * $prop30, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t7 * $prop30, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t8 * $prop30, $currency).'</td>
					<td class="text-right">'.Tools::displayPrice($this->t9 * $prop30, $currency).'</td>
				</tr>
			</table>
		</div>';
	...
	private function getRealCA()
	{
		...
		foreach ($languages as $language)
			$lang_values .= 'SUM(IF(o.id_lang = '.(int)$language['id_lang'].', total_paid_tax_incl / o.conversion_rate, 0)) as '.pSQL($language['iso_code']).',';
		...
		$sql = 'SELECT z.name, SUM(o.total_paid_tax_incl / o.conversion_rate) as total, COUNT(*) as nb
				FROM `'._DB_PREFIX_.'orders` o
		...
		$sql = 'SELECT cu.name, SUM(o.total_paid_tax_incl / o.conversion_rate) as total, COUNT(*) as nb
				FROM `'._DB_PREFIX_.'orders` o
		...
		$sql = 'SELECT SUM(total_paid_tax_incl / o.conversion_rate) as total, COUNT(*) AS nb
				FROM `'._DB_PREFIX_.'orders` o
				WHERE o.valid = 1
					AND o.`invoice_date` BETWEEN '.ModuleGraph::getDateBetween().'
					'.Shop::addSqlRestriction(Shop::SHARE_ORDER, 'o');
...
function statsindividual_sort($a, $b)
{
